...
- [ ] **Drag-to-reorder:** dragging any of the twelve reorderable icons (Cab IR, EQ, Compressor, Delay/Reverb, Boost, Graphic EQ, Chorus, Flanger, Phaser, Tremolo, Auto-Wah, Octaver) left/right in the chain row to a new position changes the actual signal chain order (audibly — e.g. Auto-Wah before vs. after Boost sounds different); Gate and Amp stay fixed at the front, Output stays fixed at the end; order persists across reloads and works the same whether the icon wraps to a second row or not.
...
- [ ] **Cycle key (GP-14):** pressing the configured key (default `\`) while Tone Lab or Play Along is open advances to the next chain entry and wraps back to the first after the last one; the key does nothing with 0 or 1 chain entries; it's inert on the Mixer and while a text field has focus; "Change…" captures the next key pressed (Esc cancels) and the new key is remembered per song; switching presets this way has no audible pop/click on a same-capture swap, and shows as a longer (but still clean) transition when the new preset loads a different NAM capture or IR.
