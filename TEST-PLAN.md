...
- [ ] **Chord ribbon vs a real chord chart (CD-8, research/chord-lane-cd8.md):** the pass that finally addressed "it's still too detailed and doesn't follow the chords properly", measured against Airbourne's *Too Much, Too Young, Too Fast* and its Ultimate Guitar chart. Re-analyse that song (ANALYSIS_VERSION → 16, so re-selecting it recomputes) and check the ribbon against the chart: each **verse should be a single A5 chip roughly 35 seconds long** (the chart writes A5 over all eight lines — before this change the same verse drew eleven chips cycling A5/Am/F/A/Bm/E); the two **pre-choruses should look identical to each other** (G5 → E5 → A5); and the roots across the whole song should be only **A, D, E, G and C** — an F, F# or B chip anywhere is a phantom and worth reporting. Expect roughly 35 chips and 10 distinct chord names against the chart's ~30 changes and 5 chords — all five chart chords appear, and the extra names (**A, Am, D, D7, E**) are quality errors on *correct* roots, mostly near the outro. Those are known remaining errors, not new ones. Then check the general behaviour this bought: a chord that holds should be **one chip that says one thing** — if you see the *same root* flipping label (A5 → A → Am → A7) while the music sits on one unchanging chord, that is the exact bug CD-8 removed and a regression worth reporting with the timestamp. Run `python3 scripts/chord_bench.py --stems <dir> --truth research/chord-truth/<song>.json --ribbon` to get the numbers rather than eyeballing.
