...
- [ ] **Drag-to-reorder:** dragging any of the twelve reorderable cards (Cab IR, EQ, Compressor, Delay/Reverb, Boost, Graphic EQ, Chorus, Flanger, Phaser, Tremolo, Auto-Wah, Octaver) by its ⠿ handle to a new position changes the actual signal chain order (audibly — e.g. Auto-Wah before vs. after Boost sounds different); Gate and Amp stay fixed at the front, Output stays fixed at the end; order persists across reloads.
- [ ] **Signal-flow arrows:** an arrow is visible between each consecutive pair of cards in chain order; dragging a card to reorder, collapsing/expanding a card, and resizing the window (including crossing the 900px single-column breakpoint) all redraw the arrows to match the new layout without a page reload; no arrow ever points to/from a nonexistent card.
