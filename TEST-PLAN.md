...
- [ ] **Chord ribbon vs a real chord chart (CD-8/CD-9, research/chord-lane-cd8.md):** the pass that addressed "it's still too detailed and doesn't follow the chords properly". Re-analyse (ANALYSIS_VERSION → 17, so re-selecting recomputes) and check against the charts in `research/chord-truth/`. Run `python3 scripts/chord_bench.py --stems <dir> --truth research/chord-truth/<song>.json --ribbon` for numbers rather than eyeballing. **Two songs, deliberately at opposite extremes — check BOTH, because each is blind to the other's failure:**
