...
- [ ] **Real-song CD-2 re-check ("Too Much, Too Young, Too Fast" — first real report found the gate only half-worked):** a genuine power-chord riff should read as "5" now, not "7" — the earlier gate only ever suppressed "5", never stopped maj/min/7 from winning anyway when a third was genuinely absent, and real distorted guitar's incidental harmonic/distortion energy near a flat 7th let "7" keep winning. If it's still showing "7" over an audibly bare power-chord riff after pulling this fix, that's a real regression worth a fresh report (with which section of the song), not expected behavior.
- [ ] **Chord-lane density sanity check:** eyeball roughly how many distinct chip changes appear across a whole verse+chorus — if it reads as one or two giant chip runs holding for 20+ seconds where you can hear several real chord changes, that's the Viterbi smoothing (`CHORD_SELF_TRANSITION_P`) over-merging and worth reporting with an approximate real chord-change count, since it hasn't been tuned against real audio yet (only synthetic tests so far). AI Lab's Follow mode has nothing to move between if the lane itself has been over-merged into one region, so a "Follow doesn't seem to follow" report and an over-static chord lane report likely share this one root cause.
