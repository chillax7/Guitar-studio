...
- [ ] **Section ribbon (BT-20, song-section-detection-spec.md):** on a song with clear structure (verses vs choruses, or an obvious drums-in/drums-out contrast), a row of colored labeled blocks ("Sections") appears above the chord ribbon. Check: blocks span the whole song without gaps; clicking a block jumps the playhead to its start; **repeated parts share a letter AND a color** (both verses show the same letter/color, both choruses another) — this is the main thing to eyeball. It's assistive/approximate: boundaries within a second or two of where you'd mark them is expected, not a bug; a spurious short block at a fade-out/outro is a known first-cut rough edge. Labels are A/B/C by repetition, deliberately NOT "Verse/Chorus" names. Requires re-selecting/re-separating a song analyzed before this build (ANALYSIS_VERSION bumped to 12). Report songs where structure is obviously wrong (whole song one block, or a wall of tiny blocks) with a rough description of the real structure. The ribbon should be absent (not empty/broken) on a very short clip or a single unbroken texture; it stays put while scrolling and rescales with zoom like the chord ribbon.
