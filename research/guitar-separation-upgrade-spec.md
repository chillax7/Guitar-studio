...
| **D. Enhance the heuristic with additional non-ML signal cues** (e.g. note-density/onset-rate — rhythm parts tend toward static chords locked to the beat grid, lead parts toward sparser, pitch-bent, more rhythmically free lines) | A rules-based upgrade, not a learned model | **✅ SHIPPED v3.2** as `split-guitar --method hybrid` — sharpens `spectral_pan_split`'s per-bin centeredness using onset-to-beat-grid alignment (`hybrid_pan_split` in backing_track.py). Falls back to plain `spectral` when there's no beat grid. Not validated against the 5-song test set yet — same "judge by listening" caveat as the other two methods |
| **F. Cancel a clean `spectral` "center" estimate directly out of the original channels** (real user report: on a track where the lead is genuinely dead-center, `spectral_pan_split`'s "center" was close to a clean lead-only proxy, but its "sides" still carried strong lead bleed) | A rules-based upgrade, not a learned model. Two magnitude-only spectral-subtraction attempts were tried and rejected by ear first (still audibly full of lead bleed, or collapsed into near-silent "squeaks" when pushed harder) before landing on a phase-coherent local Wiener-style cancellation, which only removes content actually phase-correlated with the reference | **✅ SHIPPED** as `split-guitar --method coherent` (`coherent_subtract`/`coherent_pan_split` in backing_track.py) — window=3 frames, gain cap=1.6, tuned and confirmed usable against real separated stems. Unlike the other three methods, "sides" here is genuinely independent stereo (each channel cancelled against the reference on its own), not a synthesized L/-R pair |
